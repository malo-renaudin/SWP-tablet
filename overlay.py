...
class WacomOverlay(QWidget):
    def __init__(self, udp_port=5005, fade_interval=30, fade_alpha=20):
        ...
        # Canvas for fading effect (optional)
    ...
    def listen_udp(self):
        ...
        while True:
            ...
            t = time.time()
            try:
                trigger = int(data[0]) if len(data) == 1 else int(data.decode())
            except ValueError:
                trigger = data.decode()
            print(f"Trigger received: {trigger} at {t:.6f}")
            self.data.append({"timestamp": t, "x": None, "y": None, "pressure": None, "event": f"trigger_{trigger}"})

    # -------------------- Fading (optional) --------------------
    def fade(self):
        painter = QPainter(self.image)
        painter.fillRect(self.rect(), QColor(255, 255, 255, self.fade_alpha))  # mostly transparent
        painter.end()
        self.update()
    ...
    def tabletEvent(self, event: QTabletEvent):
        ...
        if event.type() == QTabletEvent.TabletPress:
            ...
            pen = QPen(Qt.red)  # just for debugging, you can make invisible
...
if __name__ == "__main__":
    ...
    w = WacomOverlay()
    print("Wacom overlay ready on top. Start experiment in Matlab.")
    try:
        sys.exit(app.exec_())
    finally:
        w.save_csv()
